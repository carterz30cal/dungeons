...
public class RuneRoom extends CryptRoom
{
	...
	public boolean rare;
	public RuneRoom(int id, int ox, int oz, int[] c1, int[] c2, int ylevel, CryptLootTable l)
	{
		...
		if (rare) rune = ItemBuilder.i.build(RandomFunctions.get(runes_rare), 1);
	...
	public void generateBarrels(int ox,int oz,ArrayList<Block> removal)
	{
		...
		if (rare) m = Material.NETHER_BRICKS;
		for (int n : ys)
		{
